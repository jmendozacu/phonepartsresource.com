...
class Mivec_Shipping_Model_Rate_Request extends Mage_Shipping_Model_Carrier_Abstract implements Mage_Shipping_Model_Carrier_Interface
{
    ...
    {
		//helper
		$this->_quoteHelper = Mage::helper('ship/quote');
...
*/
    ...
    protected function getShippingCost(Mage_Shipping_Model_Rate_Request $request , $_carrier)
    {
		...
		//get shipping quote
		$_quoteCollection = $_quoteHelper->getShippingCollection()
			->addAttributeToFilter('carrier_id' , $_carrier['id'])
			->addAttributeToFilter('country_id' , $_countryData['id']);
		$_quoteData = $_quoteCollection->getItems()[0];
		//print_r($_quoteData);exit;

		$shippingPrice = 0;
        try {
			$shippingPrice = $this->calculatePrice($request , $_quoteData);
